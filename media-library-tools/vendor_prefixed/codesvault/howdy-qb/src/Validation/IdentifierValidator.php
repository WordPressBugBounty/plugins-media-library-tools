...
namespace TinySolutions\mlt\Vendor\CodesVault\Howdyqb\Validation;
...
class IdentifierValidator
{
    ...
    private const VALID_OPERATORS = ['=', '!=', '<>', '<', '>', '<=', '>=', 'LIKE', 'NOT LIKE', 'IN', 'NOT IN', 'BETWEEN', 'NOT BETWEEN', 'IS', 'IS NOT', 'REGEXP', 'NOT REGEXP', 'EXISTS', 'NOT EXISTS'];
    public static function validateTableName(string $tableName): string
    {
        if (strlen($tableName) > self::MAX_IDENTIFIER_LENGTH) {
            throw new \InvalidArgumentException(sprintf('Table name exceeds maximum length of %d characters.', self::MAX_IDENTIFIER_LENGTH));
        }
        if (!self::isValidIdentifier($tableName)) {
            throw new \InvalidArgumentException(sprintf('Invalid table name: "%s". Table names must start with a letter or underscore and contain only alphanumeric characters and underscores.', $tableName));
        }
    ...
    public static function validateColumnName(string $columnName): string
    {
        ...
        if (strpos($columnName, '.') !== \false) {
            ...
            if (count($parts) !== 2) {
                throw new InvalidArgumentException(sprintf('Invalid column name format: "%s". Use "table.column" or just "column".', $columnName));
            }
            if (!self::isValidIdentifier($parts[0]) || !self::isValidIdentifier($parts[1])) {
                throw new InvalidArgumentException(sprintf('Invalid column name: "%s".', $columnName));
            }
            return self::escapeIdentifier($parts[0]) . '.' . self::escapeIdentifier($parts[1]);
        }
        if (strlen($columnName) > self::MAX_IDENTIFIER_LENGTH) {
            throw new InvalidArgumentException(sprintf('Column name exceeds maximum length of %d characters.', self::MAX_IDENTIFIER_LENGTH));
        }
        if (!self::isValidIdentifier($columnName)) {
            throw new InvalidArgumentException(sprintf('Invalid column name: "%s". Column names must start with a letter or underscore and contain only alphanumeric characters and underscores.', $columnName));
        }
    ...
    public static function validateOperator(string $operator): string
    {
        ...
        if (!in_array($operator, self::VALID_OPERATORS, \true)) {
