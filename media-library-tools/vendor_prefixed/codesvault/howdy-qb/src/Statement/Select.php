...
use TinySolutions\mlt\Vendor\CodesVault\Howdyqb\Clause\JoinClause;
use TinySolutions\mlt\Vendor\CodesVault\Howdyqb\Clause\SqlCore;
use TinySolutions\mlt\Vendor\CodesVault\Howdyqb\Clause\WhereClause;
use TinySolutions\mlt\Vendor\CodesVault\Howdyqb\SqlGenerator;
...
class Select implements SelectInterface
{
    ...
    private function driver_execute($sql, $placeholders)
    {
        ...
        if (class_exists('wpdb') && $driver instanceof \wpdb) {
            ...
            return $driver->get_results($driver->prepare($sql, $placeholders), \ARRAY_A);
        }
    ...
    public function getSql()
    {
        ...
        $query = ['query' => SqlGenerator::select($this->sql), 'params' => $this->params];
        return $query;
    }
